...
// JSONObject marshals arbitrary objects into JSON. It allows us to
// eagerly validate a JSON object in order to include it in a builder
// with Builder.SetRawField.
type JSONObject struct {
	buf []byte
}

func NewJSONObject(value interface{}) (*JSONObject, error) {
	buf, err := json.Marshal(value)
	if err != nil {
		return nil, err
	}
	return &JSONObject{buf: buf}, nil
}
...
// SetRawField sets a raw JSON field in the underlying map.
func (bb *Builder) SetRawField(key string, value *JSONObject) *Builder {
	bb.m[key] = value
	return bb
}
